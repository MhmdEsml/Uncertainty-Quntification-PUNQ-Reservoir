...
%     while condition > 0.1
        ...
        %% model optimization
        m=model_optimization(m,m_uc',d_uc,d_sim,G,C_D,C_M);
...
%% generate maximume a posteriori
m_map=m;

%% estimate the posteriori covariance matrix
C_M_prime=inv(inv(C_M)+G'*inv(C_D)*G);

%% choleski decomposition
L_M_prime=chol(C_M_prime);
...
for i=1:n
    ...
    realization(:,i)=m_map+L_M_prime*random('normal',0,1,numel(m_prior),1);
